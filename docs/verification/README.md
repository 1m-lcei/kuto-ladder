...
この移行検証時点ではmainへの統合、push、Pagesへの公開は行っていません。
その後のUI調整を含む `9b39e21` までを、2026-09-20にmainへ統合しました。
...
`base-select` 導入後、閉じた選択欄が文字の上寄せと選択内容に応じた幅になっていました。
縦中央揃え・折り返しなし・一定幅（日本語ラベル118px、絵文字のみ58px）を指定し、
矢印は細い山形へ戻しました。開いたリストの配色・余白・枠線は維持しています。
上記画像を修正後の表示へ更新しました。

Edgeで両テーマ×6幅×3戦略の閉じた欄を確認し、375pxの全6表示も画像で目視確認。
選択欄・順位入力欄の位置と寸法が変わらず、折り返し・欠け・横溢れなし。
開いたリストのメニューとの装飾一致と画面内配置も確認しました。
機能テストに全戦略切替時の選択欄/入力欄の寸法維持チェックを追加し、全12項目が合格。
lint・全6 Bunテスト（60,592 assertions）・buildも合格しました。
